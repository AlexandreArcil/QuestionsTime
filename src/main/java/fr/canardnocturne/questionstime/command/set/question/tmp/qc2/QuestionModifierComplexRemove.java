...
public class QuestionModifierComplexRemove<B, T, W, V extends Collection<W>> extends QuestionModifierComplex<B, T, V> {

    private final QuestionComponentComplexCollection<B, T, W, V> questionComponent;

    public QuestionModifierComplexRemove(final QuestionRegister questionRegister, final QuestionPool questionPool,
    ...
    protected void modify(final T type, final B builder, final V values) {
        ...
        this.questionComponent.removeAll(type, builder, values);
        ...
        if (values.size() != expectedSize) {
            ...
            throw new IllegalArgumentException(componentNameError + " '"
                    + String.join(", ", stringValues) + "' is/are not present in the question");
        }
    }
}
