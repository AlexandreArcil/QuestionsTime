...
public class SetQuestionMalusCommandsExecutor implements CommandExecutor {
    ...
    public static final Parameter ACTIONS = Parameter.seq(ADD_REMOVE_ACTION, COMMAND);

    @Override
    public CommandResult execute(final CommandContext context) throws CommandException {
        final String action = context.requireOne(ADD_REMOVE_ACTION);
        final Integer command = context.requireOne(COMMAND);
        context.sendMessage(Component.text("Set command " + command + " with action " + action));
