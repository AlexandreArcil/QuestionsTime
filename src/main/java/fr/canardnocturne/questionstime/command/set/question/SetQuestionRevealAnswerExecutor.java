...
public class SetQuestionRevealAnswerExecutor implements CommandExecutor {

    public static final Parameter.Value<Boolean> REVEAL_ANSWER = Parameter.bool().key("reveal-answer").build();
    ...
    public SetQuestionRevealAnswerExecutor(final Parameter.Value<Question> specificQuestionParameter, final QuestionModifier questionModifier, final QuestionPool questionPool, final QuestionRegister questionRegister) {
    ...
    public CommandResult execute(final CommandContext context) throws CommandException {
        ...
        final boolean revealAnswer = context.requireOne(REVEAL_ANSWER);
        try {
            final Question modifiedQuestion = this.questionModifier.set(question, QuestionComponent.REVEAL_ANSWER, revealAnswer);
            this.questionRegister.replace(question, modifiedQuestion);
            this.questionPool.replace(question, modifiedQuestion);
            if(revealAnswer){
                context.sendMessage(TextUtils.composed("The answer(s) will now be revealed at the end of the question !"));
            } else {
                context.sendMessage(TextUtils.composed("The answer(s) will no longer be revealed at the end of the question !"));
            }
