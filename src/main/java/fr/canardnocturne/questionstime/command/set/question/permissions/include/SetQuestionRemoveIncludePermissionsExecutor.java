...
public class SetQuestionRemoveIncludePermissionsExecutor implements CommandExecutor  {

    private final Parameter.Value<Question> specificQuestionParameter;
    private final Parameter.Value<String> includePermissionsParameter;
    private final QuestionModifier questionModifier;
    private final QuestionPool questionPool;
    private final QuestionRegister questionRegister;

    public SetQuestionRemoveIncludePermissionsExecutor(final Parameter.Value<Question> specificQuestionParameter, final Parameter.Value<String> includePermissionsParameter, final QuestionModifier questionModifier, final QuestionPool questionPool, final QuestionRegister questionRegister) {
        this.specificQuestionParameter = specificQuestionParameter;
        this.includePermissionsParameter = includePermissionsParameter;
    ...
    public CommandResult execute(final CommandContext context) throws CommandException {
        ...
        final Collection<String> permissions = Collections.unmodifiableCollection(context.all(this.includePermissionsParameter));
        try {
            final Question modifiedQuestion = this.questionModifier.remove(question, QuestionComponent.INCLUDE_PERMISSIONS, permissions);
            this.questionRegister.replace(question, modifiedQuestion);
            this.questionPool.replace(question, modifiedQuestion);
            if(permissions.size() == 1) {
                context.sendMessage(TextUtils.composed("Include permission ", permissions.iterator().next(), " removed !"));
            } else {
                context.sendMessage(TextUtils.composed("Include permissions ", String.join(", ", permissions), " removed !"));
            }
