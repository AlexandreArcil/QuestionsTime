...
public class SetQuestionAddExcludePermissionsExecutor implements CommandExecutor {
    ...
    public SetQuestionAddExcludePermissionsExecutor(final Parameter.Value<Question> specificQuestionParameter,
    ...
    public CommandResult execute(final CommandContext context) throws CommandException {
        ...
        try {
            ...
            final Question modifiedQuestion = this.questionModifier.add(question, QuestionComponent.EXCLUDE_PERMISSIONS, permissions);
            this.questionRegister.replace(question, modifiedQuestion);
            this.questionPool.replace(question, modifiedQuestion);
            final Set<String> permissionsSet = modifiedQuestion.getExcludePermissions();
            final Set<String> newPermissionsSet = permissionsSet.stream()
                    .filter(permission -> !existingPermissions.contains(permission))
                    .collect(Collectors.toSet());
            if(newPermissionsSet.isEmpty()) {
                context.sendMessage(TextUtils.composed("No new exclude permissions to add."));
            } else if(newPermissionsSet.size() == 1) {
                context.sendMessage(TextUtils.composed("Exclude permission ", newPermissionsSet.iterator().next(), " added !"));
            } else {
                context.sendMessage(TextUtils.composed("Exclude permissions ", String.join(", ", newPermissionsSet), " added !"));
            }
