...
public class SetQuestionListComponentComplexExecutor implements CommandExecutor {

    private final Parameter.Value<Question> specificQuestionParameter;
    private final ListQuestionComponent questionComponent;

    public SetQuestionListComponentComplexExecutor(final Parameter.Value<Question> specificQuestionParameter,
    ...
    public CommandResult execute(final CommandContext context) throws CommandException {
        ...
        final Collection<String> componentValues = this.questionComponent.getComponentExtractor().apply(question);
        ...
        if(componentValues.isEmpty()) {
            ...
            message.append(TextUtils.normalWithPrefix(StringUtils.capitalize(componentNamePlural) + ": "))
                    ...
                    .append(Component.join(JoinConfiguration.newlines(), componentValues.stream().map(componentValue -> {
                                final String valueComponentCommand = this.questionComponent.isValueDoubleQuote() ? "\"" + componentValue + "\"" : componentValue;
